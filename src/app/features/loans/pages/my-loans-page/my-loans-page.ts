...
@Component({
  ...
  imports: [LoansTable, ReservationsTable],
...
export class MyLoansPage {
  ...
  protected async onCancelReservation(reservation: ReservationItem): Promise<void> {
    const confirmed = await this.ui.confirm({
      message: `¿Cancelar tu reserva de "${reservation.bookTitle}"? Perderás tu lugar en la fila de espera.`,
    ...
    this.reservationService.cancel(reservation.id).subscribe({
      next: () => this.ui.success('Reserva cancelada.'),
      error: () => this.ui.error('No se pudo cancelar la reserva. Intenta de nuevo.'),
    });
  }
}
